...
When configuring any of the above nodes, you will need to specify the hub. This is a configuration node that represents a connect to your Ezlo hub(s).

When you first make a new hub connection, the required data is the hub serial number and an Ezlo cloud username and password (you can create one just for use in *Node-Red*, so you're not using your "master" account credentials, if you wish). With just these three pieces of information, the node should be able to connect to your hub through the Ezlo cloud relay.

Connecting through the Ezlo cloud relay isn't ideal for production use, though, because it creates a high dependency on Internet access and the uptime of Ezlo's cloud services. To mitigate a majority of that, you can additionally provide the IP address of your hub in the `Local IP Address` field. Your hub *must* have a static IP address or DHCP reservation so that its address never changes. When this field is set, the node will only use the Ezlo cloud to obtain the required *local access token*, and then connect to the hub API via a WebSocket to the local IP address given. The local access token is long-lived, so it should not be necessary to refresh it very often, and this reduces (but does not entirely eliminate) the cloud dependency.

To fully eliminate the cloud dependency, a trade-off must be made: you can configure your hub for *offline anonymous access*, under which the hub allows connections locally to its API WebSocket with no authentication whatsoever. This has the advantage of not requiring Internet or Ezlo cloud access to connect to the hub at any time, but comes at the expense of security of the hub. Only you can decide if that trade-off is worth it. To enable anonymous access, you will first need to have authenticated access either via Ezlo's cloud relay (without `Local IP Address`) or locally (with `Local IP Address`). Then, you can either [set it through the Ezlo API yourself manually](), or let the hub configuration node do it by setting the `Offline Anonymous Access` field under *Options* `Enable Anonymous Access`. Then save and deploy. The node will log in and connect to the hub, set the flag, and then reboot the hub. When the hub comes back, it will reconnect, but still using authentication. At this point, you can remove the `username` from the hub configuration node, and this is the signal to the hub configuration node to not use the cloud and just connect to the hub directly. You can leave the `password` field set in case you need it later. If you later decide to disable anonymous access and return to full authentication, all you need to do is restore the `username` value and set the `Offline Anonymous Access` option to `Disable Anonymous Access`.
