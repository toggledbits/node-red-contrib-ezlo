...
module.exports = class EzloClient {
    ...
    async _start() {
        ...
        try {
            if ( AUTH_NONE !== this.require_auth ) {
                if ( ! ( loginfo[ this.config.username ] && this.authinfo ) ) {
                    debug( `ezlo: getting access credentials and endpoint, require_auth=`, this.require_auth );
                    await this._get_access( this.config.username, this.config.password, String( this.config.serial ) );
                } else {
    ...
    async _cloud_auth( username, password, serial ) {
        ...
        /* New cloud auth */
        delete loginfo[ username ];
        self.authinfo = false; /* invalidate token, too */

        loginfo[ username ] = new Promise( (resolve,reject) => {
        ...
        return await loginfo[ username ];
    }

    _get_access( username, password, serial ) {
        const self = this;
        if ( this.authinfo && this.authinfo.endpoint === this.endpoint ) {
            ...
            return new Promise( ( resolve, reject ) => {
                self._cloud_auth( username, password, serial ).then( ll => {
                    ...
                    self.fetchJSON( requrl,
                        ...
                        self.authinfo = { endpoint: self.endpoint };
                        resolve();
                    }).catch( err => {
