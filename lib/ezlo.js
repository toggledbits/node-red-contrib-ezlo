...
module.exports = class EzloClient {
    constructor( config ) {
        ...
        if ( this.config.debug ) {
            debug = ( "function" === typeof this.config.debug ) ? this.config.debug : console.debug;
        }
    }
    ...
    _start() {
        ...
        debug( `ezlo: starting connection, require_auth=`, this.require_auth );
        let loginPromise;
        if ( AUTH_NONE === this.require_auth ) {
            loginPromise = Promise.resolve();
        } else {
            debug( `ezlo: starting cloud auth for ${this.config.username}` );
            loginPromise = this._login( this.config.username, this.config.password, String( this.config.serial ) ).then( () => {
                debug( `ezlo: cloud auth succeeded` );
            });
        }
        return loginPromise.then( () => {
            /* Have/got token, connect local WebSocket to hub */
            debug( `ezlo: hub connection to ${this.config.serial} at ${this.endpoint}` );
    ...
    _retry_connection() {
        ...
        if ( ! this.stopping ) {
            ...
            let delay = Math.min( 120000, Math.max( 5000, ( this.retries - 5 ) * 15000 ) );
        ...
        return new Promise( async ( resolve, reject ) => {
            ...
            if ( loginfo[ self.config.username ] ) {
                ...
                if ( ll.expires > Date.now() ) {
                    ...
                    debug( "ezlo: re-using existing cloud auth for user", self.config.username );
            ...
            if ( AUTH_REMOTE === self.require_auth ) {
                ...
                auth_p.then( ( ll ) => {
                    ...
                    debug( "ezlo: requesting device remote access login via", ll.server_account );
                        ...
                        self.fetchJSON( syncurl, { method: "post", timeout: 30000, headers: syncHeaders, body: JSON.stringify( syncBody ) } ).then( controllerData => {
                            ...
                            for ( const key in controllerData.data.keys ) {
                                ...
                                if ( c.meta && c.meta.target && "controller" === c.meta.target.type &&
                                    ...
                                    /* We have it! */
                                    debug( `ezlo: got local access token for serial`, serial );
                                    self.authinfo = {
                                        controller_id: cid,
